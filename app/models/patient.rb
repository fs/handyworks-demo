class Patient < ActiveRecord::Base
	...
	ZIP_FORMAT = /^\d{5}(-\d{4})?$/
	ZIP_MESSAGE = 'should be in the following format: XXXXX or XXXXX-XXXX'

	PHONE_FORMAT = /^\d{3}-\d{2}-\d{2}$/
	PHONE_MESSAGE = 'should be in the following format: XXX-XX-XX'

	validates_format_of :email, :with => /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\Z/i
	validates_format_of :social_security_number, :with => /^\d{3}[-]?\d{2}[-]?\d{4}$/i, :message => 'should be in the following format: XXX-XX-XXXX'
